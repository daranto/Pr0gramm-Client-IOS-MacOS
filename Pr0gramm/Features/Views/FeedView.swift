...
struct FeedView: View {
    ...
    // --- NEU: State für den Refresh Task ---
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            ...
            // --- MODIFIED:onChange calls triggerRefreshTask ---
            ...
            .task { // Use .task for initial setup
                 ...
                 triggerRefreshTask() // Trigger initial load via the task manager
             }
            .onDisappear { // --- NEU: Cancel Task on Disappear ---
                 refreshTask?.cancel()
                 FeedView.logger.debug("FeedView disappeared, cancelling refresh task if active.")
            } // --- ENDE NEU ---
    ...
    // --- NEU: Funktion zum Starten/Managen des Refresh Tasks ---
    private func triggerRefreshTask() {
        // Cancel any existing refresh task before starting a new one
        refreshTask?.cancel()
        FeedView.logger.debug("Cancelling previous refresh task (if any).")

        // Create and store the new task
        refreshTask = Task {
            await refreshItems()
        }
        FeedView.logger.debug("Scheduled new refresh task.")
    }
    // --- ENDE NEU ---


    // feedContentView, scrollViewContent, noFilterContentView bleiben unverändert...
    @ViewBuilder
    ...
    private var scrollViewContent: some View {
        ...
        .refreshable { await refreshItems() } // .refreshable calls the underlying function directly
    }
    ...
    /// Refreshes the feed items by fetching the latest first page from the API and replacing the current list.
    /// This function should now be primarily called via `triggerRefreshTask`.
    func refreshItems() async {
        FeedView.logger.info("RefreshItems Task started.")

        guard !Task.isCancelled else { // --- NEU: Check Cancellation ---
            FeedView.logger.info("RefreshItems Task cancelled before starting.")
            return
        } // --- ENDE NEU ---
        ...
        guard let currentCacheKey = Optional(self.feedCacheKey) else {
             FeedView.logger.error("Cannot refresh: Cache key generation failed.")
             await MainActor.run { self.errorMessage = "Interner Cache-Fehler." }
             return
        }

        // Use a temporary loading flag to avoid flickering if refresh is very fast
        var showLoadingIndicator = true
        Task { @MainActor in
            try? await Task.sleep(for: .milliseconds(250)) // Only show indicator if loading takes > 250ms
        ...
        await MainActor.run {
            ...
            // self.isLoading = true // Don't set isLoading immediately
            self.errorMessage = nil
        }
        canLoadMore = true; isLoadingMore = false

        // Defer isLoading = false AND resetting showLoadingIndicator
        defer {
            Task { @MainActor in
                self.isLoading = false
                showLoadingIndicator = false // Make sure flag is false when done
                FeedView.logger.info("Finishing refresh process (isLoading set to false via defer).")
            }
        }

        let currentApiFlags = settings.apiFlags
        let currentFeedType = settings.feedType
        FeedView.logger.info("Starting API fetch for refresh (Feed: \(currentFeedType.displayName), Flags: \(currentApiFlags)). Strategy: REPLACE.")

        do {
            let fetchedItemsFromAPI = try await apiService.fetchItems(flags: currentApiFlags, promoted: settings.apiPromoted)

            guard !Task.isCancelled else { // --- NEU: Check Cancellation ---
                FeedView.logger.info("RefreshItems Task cancelled after API fetch.")
                return
            } // --- ENDE NEU ---
            ...
            await MainActor.run {
                let oldItemCount = self.items.count
                ...
                if !navigationPath.isEmpty && (oldItemCount != fetchedItemsFromAPI.count || oldFirstItemId != newFirstItemId) {
                     navigationPath = NavigationPath()
                     FeedView.logger.info("Popped navigation due to refresh resulting in different list content.")
                }
        ...
        } catch is CancellationError { // --- NEU: Catch CancellationError ---
             FeedView.logger.info("RefreshItems Task API call explicitly cancelled.")
             // No need to set error message or change canLoadMore
        } catch {
            FeedView.logger.error("API fetch failed during refresh: \(error.localizedDescription)")
            // Check cancellation again before setting error state
            guard !Task.isCancelled else {
                FeedView.logger.info("RefreshItems Task cancelled after API error.")
                return
            }
            await MainActor.run {
    ...
    // getIdForLoadMore und loadMoreItems bleiben unverändert...
    ...
    func loadMoreItems() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                if newItems.isEmpty { FeedView.logger.info("Reached end of feed (API returned empty list for older than \(olderValue))."); canLoadMore = false }
                else { let currentIDs = Set(self.items.map { $0.id }); let uniqueNewItems = newItems.filter { !currentIDs.contains($0.id) }; if uniqueNewItems.isEmpty { FeedView.logger.warning("All loaded items (older than \(olderValue)) were duplicates."); canLoadMore = false; FeedView.logger.info("Assuming end of feed because only duplicates were returned.") } else { self.items.append(contentsOf: uniqueNewItems); appendedItemCount = uniqueNewItems.count; FeedView.logger.info("Appended \(uniqueNewItems.count) unique items to original list. Total items: \(self.items.count)"); self.canLoadMore = true } }
            }
            ...
            await MainActor.run { guard self.isLoadingMore else { return }; if items.isEmpty { errorMessage = "Fehler beim Nachladen: \(error.localizedDescription)" }; canLoadMore = false }
        }
    }
}
