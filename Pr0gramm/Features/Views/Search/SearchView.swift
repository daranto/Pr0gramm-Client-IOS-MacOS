...
struct SearchView: View {
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            ...
            .task { await playerManager.configure(settings: settings) }
            ...
            .onChange(of: searchText) { oldValue, newValue in if hasSearched && newValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && !isLoading { Task { @MainActor in items = []; hasSearched = false; errorMessage = nil; didPerformInitialPendingSearch = false } } }
    ...
    private func performSearch() async {
        let trimmedSearchText = searchText.trimmingCharacters(in: .whitespacesAndNewlines); guard !trimmedSearchText.isEmpty else { SearchView.logger.info("Search skipped: search text is empty."); await MainActor.run { items = []; hasSearched = false; errorMessage = nil; isLoading = false; didPerformInitialPendingSearch = false }; return }
        SearchView.logger.info("Performing search for tags: '\(trimmedSearchText)'"); await MainActor.run { isLoading = true; errorMessage = nil; items = []; hasSearched = true }; defer { Task { @MainActor in self.isLoading = false } }
        do {
            let fetchedItems = try await apiService.fetchItems(flags: settings.apiFlags, tags: trimmedSearchText) // Use correct API call
            let currentSearchText = await MainActor.run { self.searchText.trimmingCharacters(in: .whitespacesAndNewlines) }
            if currentSearchText == trimmedSearchText { await MainActor.run { self.items = fetchedItems }; SearchView.logger.info("Search successful, found \(fetchedItems.count) items for '\(trimmedSearchText)'.") }
            ...
            let currentSearchText = await MainActor.run { self.searchText.trimmingCharacters(in: .whitespacesAndNewlines) }
            if currentSearchText == trimmedSearchText { await MainActor.run { self.errorMessage = "Fehler: \(error.localizedDescription)"; self.items = [] } }
